...
def test_dashboard_endpoints():
    ...
    r = c.post(
        "/api/run",
        json={"instruction": "Roll up all issues into a triage digest grouped by priority"},
    )
...
def test_composition_dsl_dry_run_does_not_mutate():
    ...
    spec = CapabilitySpec(
        ...
        composition=[
            ...
            TransformStep(
                op="transform:group", args={"source": "{{i}}", "by": "priorityLabel"}, bind="g"
            ),
            TransformStep(
...
def test_cli_run_offline(tmp_path):
    ...
    res = CliRunner().invoke(
        app, ["run", "--offline", "--memory", db, "Create a bug titled 'Boom' in Engineering"]
    )
