...
def test_constraint_accessors_round_trip():
    m = Memory(":memory:")
    m.capability.upsert_constraint(Constraint(kind=ConstraintKind.ENTITY_ID, scope="team",
                                              key="Engineering", value="team_eng"))
    m.capability.upsert_constraint(Constraint(kind=ConstraintKind.ENUM, scope="field",
                                              key="issue.priority", value={"range": [0, 4]}))
    m.capability.upsert_constraint(Constraint(kind=ConstraintKind.PERMISSION, scope="mutation",
                                              key="issueDelete", value="forbidden"))
...
def test_only_runtime_learned_tagged_as_learning():
    m = Memory(":memory:")
    m.capability.upsert_constraint(Constraint(kind=ConstraintKind.WORKFLOW_RULE, scope="team", key="t",
                                              value={"requires": "estimate"}, rewrites_plan=True,
...
def test_memory_persists_across_connections(tmp_path):
    ...
    m1 = Memory(path)
    m1.capability.upsert_constraint(Constraint(kind=ConstraintKind.ENTITY_ID, scope="team",
                                              key="Engineering", value="team_eng"))
    m1.close()
...
def test_snapshot_and_diff():
    m = Memory(":memory:")
    before = m.snapshot()
    m.capability.upsert_constraint(Constraint(kind=ConstraintKind.ENTITY_ID, scope="team", key="E", value="t"))
    after = m.snapshot()
