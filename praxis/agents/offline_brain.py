...
def _title(text: str) -> str:
    ...
    cleaned = re.sub(
        r"^(create|file|add|open|log|raise|make)\s+(a|an|the)?\s*", "", text, flags=re.I
    )
    cleaned = re.sub(
        r"\b(in|for|to)\s+(the\s+)?(engineering|design)\s*(team)?\b.*$", "", cleaned, flags=re.I
    )
...
def _build_plan(instruction: str, available_caps: set[str] | None = None) -> str:
    ...
    if verb == "aggregate":
        ...
        title = (
            _title(instruction) if _QUOTE_RE.search(instruction) else f"Issue digest by {by_label}"
        )
        ...
        return _plan(
            [
                {
                    "intent": f"build a digest document grouped by {by_label}",
                    "capability": cap,
    ...
    if verb == "create":
        steps = [
            {
                "intent": "resolve the team",
                "capability": "find_team",
                "args": {"name": _team(instruction)},
            }
        ]
    ...
    if verb == "query":
        ...
        return _plan(
            [{"intent": "query issues", "capability": "query_issues", "args": {"filter": f}}],
    ...
    if ident:
        steps.append(
            {
                "intent": f"resolve issue {ident.group(1)}",
                "capability": "find_issue",
        ...
        steps.append(
            {
                "intent": "find my in-progress issues",
                "capability": "query_issues",
                "args": {"filter": {"assigneeId": "{{step0.id}}", "stateType": "started"}},
                "depends_on": [0],
            }
        )
    ...
    # set estimate
    mest = re.search(r"estimat\w*\s+(?:it\s+)?(?:at\s+|to\s+|of\s+)?(\d+)", low) or re.search(
        r"(\d+)\s*(?:points|story points|pts)", low
    )
    if mest:
        steps.append(
            {
                "intent": "set the estimate",
                "capability": "update_issue",
                "args": {"id": target, "estimate": int(mest.group(1))},
                "depends_on": [base],
            }
        )
    ...
    if any(w in low for w in ("done", "complete", "close", "finish")):
        steps.append(
            {
                "intent": "resolve the Done state",
                "capability": "find_workflow_state",
                "args": {"teamId": team_ref, "type": "completed"},
                "depends_on": [base],
            }
        )
        ds = nxt() - 1
        steps.append(
            {
                "intent": "move the issue to Done",
                "capability": "update_issue",
                "args": {"id": target, "stateId": f"{{{{step{ds}.id}}}}"},
                "depends_on": [base, ds],
            }
        )
    # move to in progress / start
        ...
        w in low
        for w in ("in progress", "in-progress", "start working", "pick up", "picked up", "start ")
    ):
        steps.append(
            {
                "intent": "resolve the In Progress state",
                "capability": "find_workflow_state",
                "args": {"teamId": team_ref, "name": "In Progress"},
                "depends_on": [base],
            }
        )
        ss = nxt() - 1
        steps.append(
            {
                "intent": "move the issue to In Progress",
                "capability": "update_issue",
                "args": {"id": target, "stateId": f"{{{{step{ss}.id}}}}"},
                "depends_on": [base, ss],
            }
        )
    # comment
    ...
    if "comment" in low or "note" in low or (q and "titled" not in low):
        ...
        steps.append(
            {
                "intent": "leave a comment",
                "capability": "add_comment",
                "args": {"issueId": target, "body": body},
                "depends_on": [base],
            }
        )
    # assign to <name>
    ...
    if massign and massign.group(1) not in ("me",):
        steps.append(
            {
                "intent": f"resolve user {massign.group(1)}",
                "capability": "find_user",
                "args": {"name": massign.group(1)},
                "depends_on": [base],
            }
        )
        us = nxt() - 1
        steps.append(
            {
                "intent": "assign the issue",
                "capability": "update_issue",
                "args": {"id": target, "assigneeId": f"{{{{step{us}.id}}}}"},
                "depends_on": [base, us],
            }
        )
    ...
    if len(steps) <= (1 if ident else 2):  # nothing actionable detected → safe read-only fallback
        return _plan(
            [{"intent": "query issues", "capability": "query_issues", "args": {"filter": {}}}],
                ...
                "composition": [
                    ...
                    },
                    {
                        ...
                        "args": {
                            "source": "{{groups}}",
                            "title": "{{args.title}}",
                            "columns": ["identifier", "title", "priorityLabel"],
                        },
                        "bind": "table",
                    },
                    {
                        "op": "capability:create_document",
                        "args": {"title": "{{args.title}}", "content": "{{table}}"},
                        "bind": "doc",
                    },
                ],
        ...
        return json.dumps(
            {
                ...
                "composition": [
                    {
                        "op": "capability:query_issues",
                        "args": {"filter": "{{args.filter}}"},
                        "bind": "issues",
                    },
                    {
                        "op": "transform:create_each",
                        "args": {
                            "source": "{{issues}}",
                            "capability": "add_comment",
                            "args": {"issueId": "{{item.id}}", "body": "{{args.body}}"},
                        },
                        "bind": "posted",
                    },
                ],
...
def make_offline_responder():
    ...
    def responder(system: str, prompt: str) -> str | None:
