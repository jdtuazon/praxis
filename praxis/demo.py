...
def run_demo(console: Console, *, offline: bool = True) -> None:
    ...
    console.print(
        Panel(
            "[bold]Praxis demo[/] — 3 instructions of increasing complexity, on one persistent memory.\n"
...
def run_benchmark(console: Console, *, offline: bool = True, json_out: bool = False) -> dict:
    ...
    results["workflow_rule"] = {
        "cold": _row(a_cold),
        "warm": _row(a_warm),
        "control_after_wipe": _row(a_ctrl),
    }

    # ── Mechanism B: synthesis → transfer (capability reuse) ──────────────────
    mem2 = Memory(":memory:")
    fake2 = FakeLinear()
    b_cold = _agent(offline, mem2, fake2).run(
        "Roll up all issues into a triage digest grouped by priority"
    )
    ...
    console.print(
        "[dim]→ the wasted call vanishes once the rule is learned, and returns when the constraint is wiped: "
        "the behaviour change is caused by memory, not luck.[/]\n"
    )
    _print_bench_table(
        console,
        "Mechanism B — runtime synthesis, then transfer to a related instruction",
        [
    ...
    console.print(
        "[dim]→ the second, never-seen instruction pays no synthesis cost because the capability "
