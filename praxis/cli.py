...
app = typer.Typer(
    add_completion=False, help="Praxis — an autonomous, self-improving agent for Linear."
)
...
def run(
    ...
    offline: bool = typer.Option(
        False, "--offline", help="Use the deterministic offline brain + FakeLinear (no API keys)."
    ),
    json_out: bool = typer.Option(False, "--json", help="Emit the structured report as JSON."),
    no_memory: bool = typer.Option(
        False, "--no-memory", help="Negative control: ignore prior memory (start cold)."
    ),
    memory_path: str | None = typer.Option(None, "--memory", help="Override the memory DB path."),
):
...
def capabilities(
    ...
    for spec in sorted(agent.registry.specs(), key=lambda s: (s.source.value, s.name)):
        t.add_row(
            spec.name, spec.kind.value, spec.source.value, spec.status.value, spec.description[:60]
        )
    console.print(t)


@memory_app.command("show")
def memory_show(memory_path: str | None = typer.Option(None, "--memory")):
...
def memory_reset(
    ...
    memory_path: str | None = typer.Option(None, "--memory"),
):
...
def memory_wipe_constraints(memory_path: str | None = typer.Option(None, "--memory")):
...
def memory_compact(
    keep_recent: int = typer.Option(
        10, "--keep-recent", help="Recent runs per signature to keep verbatim."
    ),
    memory_path: str | None = typer.Option(None, "--memory"),
):
...
def serve(
    ...
    offline: bool = typer.Option(
        True,
        "--offline/--live",
        help="Back the dashboard with FakeLinear (default) or real Linear.",
    ),
):
    ...
    console.print(
        f"[bold green]Praxis dashboard[/] → http://{host}:{port}  ({'offline' if offline else 'live'})"
    )
