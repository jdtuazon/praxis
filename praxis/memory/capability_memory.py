...
class CapabilityMemory:
    ...
    def save_capability(self, spec: CapabilitySpec) -> None:
        ...
        self.store.execute(
            ...
            (spec.name, self.store.dumps(spec.model_dump(mode="json")), spec.source.value,
             spec.status.value, spec.schema_hash, spec.created_at, now),
        )
        self.store.commit()
    ...
    def list_capabilities(self, *, source: Optional[CapabilitySource] = None) -> list[CapabilitySpec]:
        if source:
            rows = self.store.query("SELECT spec_json FROM capabilities WHERE source = ? ORDER BY name", (source.value,))
        else:
    ...
    def record_capability_use(
        ...
        row = self.store.one(
            "SELECT * FROM capability_stats WHERE capability = ? AND signature = ?", (capability, signature)
        )
        if row:
            self.store.execute(
                ...
                (1 if success else 0, 0 if success else 1, duration, api_calls, self.store.now(), capability, signature),
            )
        else:
            self.store.execute(
                ...
                (capability, signature, 1, 1 if success else 0, 0 if success else 1, duration, api_calls, self.store.now()),
            )
        self.store.commit()

    def success_count(self, capability: str) -> int:
        row = self.store.one("SELECT COALESCE(SUM(successes),0) AS s FROM capability_stats WHERE capability = ?", (capability,))
    ...
    def capability_health(self, capability: str) -> dict:
        ...
        return {"attempts": a, "successes": s, "failures": f, "success_rate": (s / a) if a else None, "total_api_calls": c}

    # ── constraints (the learned world-model) ──────────────────────────────
    def upsert_constraint(self, c: Constraint) -> int:
        existing = self.store.one(
            "SELECT id, hits FROM constraints WHERE kind=? AND scope=? AND key=?", (c.kind.value, c.scope, c.key)
        )
        if existing:
            self.store.execute(
                """UPDATE constraints SET value_json=?, description=?, origin=?, rewrites_plan=?,
                   ...
                   WHERE id=?""",
                (self.store.dumps(c.value), c.description, c.origin.value, int(c.rewrites_plan),
                 c.verification_policy.value, c.ttl_seconds, self.store.now(), c.confidence,
                 c.discovered_from_execution, existing["id"]),
            )
            self.store.commit()
        ...
        cur = self.store.execute(
            """INSERT INTO constraints(kind, origin, scope, key, value_json, description, rewrites_plan,
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0)""",
            (c.kind.value, c.origin.value, c.scope, c.key, self.store.dumps(c.value), c.description,
             int(c.rewrites_plan), c.verification_policy.value, c.ttl_seconds, c.discovered_from_execution,
             c.observed_at, c.confidence),
        )
        self.store.commit()
    ...
    def get_constraints(
        ...
        *,
        ...
        kind: Optional[ConstraintKind] = None,
    ...
    def invalidate_constraint(self, constraint_id: int, *, by_execution: Optional[int]) -> None:
        self.store.execute(
    ...
    def cached_entity_id(self, scope: str, key: str) -> Optional[str]:
    ...
    def enum_constraint(self, field: str) -> Optional[Constraint]:
